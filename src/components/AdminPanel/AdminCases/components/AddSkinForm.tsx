...
export const AddSkinForm: FC<TAddSkinFormProps> = ({ setAddSkinFormActive, addItem }) => {
  const schema = yup.object().shape({
    type: yup
      ...
      .test('select', 'Выберите значение', (value) => (value === 'default' ? false : true)),
    title: yup
      .string()
    ...
    skin: yup
      .string()
      .required('Не указан скин')
      .test('select3', 'Выберите значение', (value) => (value === 'default' ? false : true)),
    rare: yup
      .string()
      .required('Не указана редкость')
      .test('select4', 'Выберите значение', (value) => (value === 'default' ? false : true)),
    quality: yup
      .string()
      .required('Не указано качество')
      .test('select5', 'Выберите значение', (value) => (value === 'default' ? false : true)),
  ...
  return (
    <Box className="add-skin-form">
      ...
      <Box className="add-skin-form__form">
        <Box>Название</Box>
        <AdminSelect defaultValue={'default'} {...register('title')} error={!!formState.errors.title}>
          <MenuItem value="default" disabled hidden sx={{ display: 'none' }}>
            Выбрать
          </MenuItem>
          <MenuItem value="default" disabled hidden sx={{ display: 'none' }}>
      ...
      <Box className="add-skin-form__form">
        <Box>Скин</Box>
        <AdminSelect defaultValue={'default'} {...register('skin')} error={!!formState.errors.skin}>
          <MenuItem value="default" disabled hidden sx={{ display: 'none' }}>
            Выбрать
          </MenuItem>
